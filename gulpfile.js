...
const jestSnapshotTests = () => {
  return function (done) {
     ...
     */
    return shell.exec(`jest --rootDir=${testPath} --testMatch='**/*.jest.test.js' --ci=0`, { async: true }, (exitCode) => {
...
// default testMatch : **/__tests__/**/*.[jt]s?(x), **/?(*.)+(spec|test).[tj]s?(x)

const jestSpecTests = () => {
  return function (done) {
    const testPath = path.join(__dirname, '/theSrc/scripts/')
...
gulp.task('jestSpecTests', jestSpecTests(gulp))
